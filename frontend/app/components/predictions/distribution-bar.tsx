...
export default function DistributionBar({distribution, homeName, awayName}: {
    ...
    return (
        <div className="mt-4">
            <div className="flex justify-between text-[10px] uppercase tracking-wider font-semibold text-slate-500 dark:text-gray-400 mb-1">
                <span className="truncate max-w-[40%]" title={`${homeShort} ${Math.round(homePct)}%`}>{homeShort} {Math.round(homePct)}%</span>
                <span title={`Draw ${Math.round(drawPct)}%`}>Draw {Math.round(drawPct)}%</span>
                <span className="truncate max-w-[40%] text-right" title={`${awayShort} ${Math.round(awayPct)}%`}>{awayShort} {Math.round(awayPct)}%</span>
            </div>
